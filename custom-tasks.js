...
export const customKeystoreFilePath = (hre) => {
  return {
    keystore: {
      filePath:
        hre.config.keystore.filePath.split("keystore.json")[0] +
        `${pkg.name}-keystore.json`,
    },
  };
};
...
export const setSecretTask = overrideTask(["keystore", "set"])
  .setAction(async (args, hre) => {
    const keystoreLoader = setupKeystoreLoaderFrom({
      config: customKeystoreFilePath(hre),
    });
    await set(
      args,
      keystoreLoader,
...
export const isSecretSetTask = task(
  ["keystore", "contains"],
  "Checks if a secret is set in the keystore"
)
  .addPositionalArgument({
    name: "key",
    type: "STRING",
    description:
      "Specifies the key that we are looking for in the keystore file",
  })
  .setAction(async (args, hre) => {
    const keystoreLoader = setupKeystoreLoaderFrom({
      config: customKeystoreFilePath(hre),
...
export const displayKeystoreFilePathTask = overrideTask(
  ["keystore", "path"],
  "Displays the keystore file path in the CLI"
)
  .setAction(async (args, hre) => {
    const keystoreLoader = setupKeystoreLoaderFrom({
      config: customKeystoreFilePath(hre),
    ...
    console.log(
      `Custom Hardhat keystore file path for ${pkg.name} project:`,
      customKeystoreFilePath(hre).keystore.filePath
    );
  })
  .build();

// Custom subtask to the built-in keystore task to ask for the keystore password in the CLI while running mocha tests
export const askPasswordTask = task(
  ["keystore", "askpwd"],
  "Asks for the keystore password in the CLI"
)
  .setAction(async (args, hre) => {
...
export const decryptSecretTask = task(
  ["keystore", "decrypt"],
  "Decrypts a secret value given a key and a password"
)
  .addPositionalArgument({
    name: "key",
    type: "STRING",
