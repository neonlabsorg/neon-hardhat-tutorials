...
const secretsKeys = ["PRIVATE_KEY_OWNER", "PRIVATE_KEY_USER_1"];
...
export async function getSecrets() {
  ...
  if (envFileSecretKeys.length) {
    ...
    if (!usingPlainTextSecrets) {
      ...
      throw new Error(
        "Please remove plain text secrets from .env file before running Hardhat tests and scripts"
      );
    }
  }
  try {
    const secrets = await asyncForLoop(
      ...
      async function (secretsKeys, index, secrets) {
        ...
        if (process.env[secretsKeys[index]]) {
          // If secret is found in .env file
          if (usingPlainTextSecrets) {
            // If user confirmed using plain text secrets in .env file
            console.log(
              "\n\u{26A0} \x1b[33mReading secret " +
                secretsKeys[index] +
                " from \x1b[36m.env" +
                "\x1b[33m file\x1b[0m\n"
            );
            if (secretsKeys[index].split("PRIVATE_KEY").length > 1) {
              try {
                if (secretsKeys[index].split("SOLANA").length > 1) {
                  // SVM private key
                  secrets.wallets[secretName] = web3.Keypair.fromSecretKey(
                    bs58.decode(process.env[secretsKeys[index]])
                  );
                  console.log(
                    "   Solana u" +
                      secretName.split("solanaU")[1] +
                      " address:",
                  ...
                  console.log(
                    "   NeonEVM " + secretName + " address:",
                    secrets.wallets[secretName].address
                  );
                }
                ...
                console.error(
                  "\n\u{26A0} \x1b[33mError: failed to load private key " +
                    secretsKeys[index] +
            ...
            // If user did not confirm using plain text secrets in .env file
            throw new Error(
              "User denied using plain text secrets from .env file"
            );
          }
        } else {
          ...
          if (!usingPlainTextSecrets) {
            ...
            // .env file
            console.log(
              "\n\u{26A0} \x1b[33mDecrypting secret " +
                secretsKeys[index] +
                " from \x1b[36m" +
            ...
            try {
              ...
              console.error(
                "\n\u{26A0} \x1b[33mError: Failed to decrypt secret " +
                  secretsKeys[index] +
    ...
    if (
      error.message === "User denied using plain text secrets from .env file"
    ) {
      // If user denied using plain text secrets from .env file
      console.log(
        "\x1b[33m Please remove plain-text secrets from \x1b[36m.env\x1b[33m file and store them using " +
          "Hardhat's encrypted keystore instead using the following CLI command:\n"
      );
      console.log(
        "\x1b[32m   npx hardhat keystore set < SECRET_KEY >\x1b[0m\n"
      );
      throw error;
...
export async function decryptSecret(ethers, secrets, secretKey, logs = true) {
  ...
  if (!secrets.keystorePassword) {
    if (process.env[keystorePasswordKey]) {
      // Keystore password can be stored in .env file
      secrets.keystorePassword = process.env[keystorePasswordKey];
    } else {
      // If not, ask for password in CLI
      secrets.keystorePassword = await hre.tasks
        .getTask("keystore")
        .subtasks.get("askpwd")
        .run();
    }
  ...
  if (secretKey.split("PRIVATE_KEY").length > 1) {
    try {
      if (secretKey.split("SOLANA").length > 1) {
        // SVM private key
        secrets.wallets[secretName] = web3.Keypair.fromSecretKey(
          bs58.decode(
        ...
        console.log(
          "   Solana u" + secretName.split("solanaU")[1] + " address:",
          secrets.wallets[secretName].publicKey.toBase58()
        );
      } else {
        // EVM private key
      ...
      console.error(
        "\n\u{26A0} \x1b[33mError: failed to decrypt and load private key " +
          secretKey +
          " from Hardhat's encrypted keystore.\x1b[0m\n"
      );
      throw error;
...
async function promptPlainTextSecretsWarning(envFileSecretKeys) {
  let warningMessage =
    "\x1b[33mThe following plain-text secrets have been found in \x1b[36m.env\x1b[33m " +
    "file: ";
...
async function checkKeystoreSecretIsSet(secretKey) {
  if (
    !(await hre.tasks
      .getTask("keystore")
      .subtasks.get("contains")
      .run({ key: secretKey }))
  ) {
    throw new Error(
      "\n\u{26A0}  \x1b[36m" +
        secretKey +
        "\x1b[33m key not found in project's keystore file. Please set it up using \x1b[36mnpx hardhat keystore set " +
        secretKey +
        "\x1b[0m"
    );
  }
